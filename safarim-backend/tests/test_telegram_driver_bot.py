...
Oqim doimiy yo'nalish shabloniga tayanadi, shuning uchun botdan faqat sana,
vaqt va qaytish so'raladi. Tarmoqqa chiqilmaydi — `_call` monkeypatch qilinadi.
"""
import uuid
...
async def test_ikkinchi_qadam_odatdagi_vaqtni_belgilaydi(db, driver_user, sent):
    ...
    assert marked[0]["callback_data"] == "pb:r:1:0800"


@pytest.mark.asyncio
async def test_qaytish_vaqti_yoq_bolsa_sorramaydi_va_elon_qiladi(db, driver_user, sent):
    """Ortiqcha bosish bo'lmasin: shablonda qaytish yo'q bo'lsa savol ham yo'q."""
    user, _ = driver_user
    await _setup(db, driver_user, return_time=None)

    await telegram_service.handle_update(db, _cq("pb:r:1:0800"))

    assert "e'lon qilindi" in _texts(sent).lower()
    trips = await _open_trips(db, user)
    assert len(trips) == 1
    assert trips[0].departure_time == time(8, 0)
...
async def test_qaytish_vaqti_bor_bolsa_soraydi(db, driver_user, sent):
    ...
    btns = _buttons(sent)
    assert [b["callback_data"] for b in btns] == ["pb:go:1:0800:1", "pb:go:1:0800:0"]
...
async def test_qaytish_bilan_ikki_safar_yaratiladi(db, driver_user, sent):
    ...
    await telegram_service.handle_update(db, _cq("pb:go:1:0800:1"))
...
async def test_takroriy_safar_xatosi_korinadi(db, driver_user, sent):
    ...
    await telegram_service.handle_update(db, _cq("pb:go:1:0800:0"))
    sent.clear()
    await telegram_service.handle_update(db, _cq("pb:go:1:0800:0"))
...
async def test_callback_data_64_baytdan_oshmaydi(db, driver_user, sent):
    ...
    for update in (_msg(bot.MENU_PUBLISH), _cq("pb:t:1"), _cq("pb:r:1:0800"),
